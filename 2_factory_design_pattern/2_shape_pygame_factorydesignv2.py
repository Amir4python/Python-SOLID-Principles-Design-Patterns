...
class ShapeFactory:
    @staticmethod
    def createShape(context:ShapeContext):
        print(f'{context=} {context.shapeType=}')
        if context.shapeType == 'Circle':
            return Circle(context.x,context.y)
        elif context.shapeType == 'Rectangle':
    ...
    while running:
        for event in pygame.event.get():
            if event.type==pygame.QUIT:
                ...
                print(f'{x=} {y=} {shapeType=}')
                context=ShapeContext(shapeType=shapeType,x=x,y=y)
                shape=ShapeFactory.createShape(context)
                shapes.append(shape)
