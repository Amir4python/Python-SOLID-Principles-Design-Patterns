...
class FileAuditManager:
    ...
    def __new__(cls,file_name='audit.log'):
        with cls._lock:
            if cls._instance is None:
                ...
                with open(cls._instance._file_name, 'a') as audit_log:
                    audit_log.write(f'{datetime.datetime.now()} FileAuditManager created\n')
    ...
    def log_message(self, message):
        with self._lock:
            with open(self._file_name, 'a') as audit_log:
                audit_log.write(f'{datetime.datetime.now()} {message}\n')

def test_file_audit_manager():
    manager=FileAuditManager('myAuditFile.log')
    manager.log_message('Test message')
